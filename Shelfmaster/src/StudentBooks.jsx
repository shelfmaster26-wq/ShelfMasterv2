...
export default function StudentBooks() {
  ...
  const [data, setData]                 = useState({ pending: [], released: [], claimed: [], overdue: [] });
  ...
  async function fetchData() {
    ...
    // FIX Bug 1: books.authors is a plain text column — no book_authors join table exists in schema
    ...
    if (res.error && isMigrationError(res.error)) {
      // Fallback: query without book_copies if that relation isn't migrated yet
    ...
    setData({ pending, released, claimed, overdue });
    setLoading(false);
  }

  async function cancelRequest(requestId) {
    setCancellingId(requestId);
    // FIX Bug 3: update status to 'cancelled' instead of deleting the row —
    // deleting breaks FK references in fines/notifications tables
  ...
  // FIX Bug 2: books.authors is a plain text column, not a nested relation
  const getAuthors = (book) => book?.authors || '—';

  // FIX Bug 4: capture `now` once to avoid midnight boundary glitches
    ...
    <div style={{ background: 'var(--cream)', minHeight: '100vh', fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
        ...
        .page-header h2 {
          ...
          font-size: 2rem; color: var(--maroon); margin: 0 0 4px; line-height: 1.2;
        }
        ...
        .tabs-bar {
          ...
          opacity:0; animation: fadeUp .45s ease .12s forwards;
          overflow-x: auto;
        }
        ...
        .loans-table td {
          ...
          border-bottom: 1px solid #f1f5f9; font-size: 0.88rem; color: #334155;
        }
        ...
        .accession-chip {
          ...
          color: #4338ca; padding: 3px 10px; border-radius: 6px;
          font-size: 0.76rem; font-family: 'Fira Code','Courier New',monospace;
          font-weight: 700; letter-spacing: 0.5px; border: 1px solid #c7d2fe;
        }
        ...
        .req-row {
          ...
          padding:16px 22px; border-bottom:1px solid #f1f5f9; gap:16px;
        ...
        .req-book-dot {
          ...
          display:flex; align-items:center; justify-content:center;
          font-size:1rem; flex-shrink:0; border:1px solid #fcd34d;
        }
        ...
        .empty-icon-ring {
          ...
          display:flex; align-items:center; justify-content:center;
          font-size:1.8rem; margin-bottom:16px; color:#cbd5e1;
        }
      ...
      <div className="books-wrap">
        ...
        <div className="page-header">
          <h2>My Books</h2>
          <p>Track your loans, requests, and borrowing history</p>
        </div>
                      ...
                      <div style={{ display:'flex', alignItems:'center', gap:12, minWidth:0 }}>
                        ...
                        <div style={{ minWidth:0 }}>
                          ...
                          <div className="req-meta">
                            by {getAuthors(req.books)} · {new Date(req.created_at).toLocaleDateString()}
                          </div>
                        </div>
                      </div>
                      ...
                      <tbody>
                        {currentRows.map(row => (
                          <tr key={row.id}>
                            ...
                            <td>
                              {row.book_copies?.accession_id ? (
                                <>
                                  ...
                                  <div className="copy-num">Copy #{row.book_copies.copy_number}</div>
                                </>
                              ) : '—'}
                            </td>
                            ...
                            <td>
                              {row.status === 'approved'
                                ? <span className="status-badge" style={{ background:'#dbeafe', color:'#1d4ed8' }}><span className="status-dot" />Approved</span>
                                : <span className="status-badge" style={{ background:'#dbeafe', color:'#1d4ed8' }}><span className="status-dot" />Approved</span>}
                            </td>
                          </tr>
                        ))}
              ...
              currentRows.length === 0 ? (
                ...
                <>
                  ...
                  <div className="loans-table-wrap" style={{ overflowX:'auto' }}>
                    <table className="loans-table">
                      <thead>
                        <tr style={{ background:'linear-gradient(135deg, #7f1d1d 0%, #b91c1c 100%)' }}>
                          <th>Book</th>
                          <th>Copy / Barcode</th>
                          <th>Borrowed</th>
                          <th>Was Due</th>
                      ...
                      <tbody>
                        {currentRows.map(loan => {
                          const now = new Date();
                          const diffDays = loan.due_date
                            ? Math.abs(Math.ceil((new Date(loan.due_date) - now) / (1000 * 60 * 60 * 24)))
                            : null;
                          return (
                            <tr key={loan.id}>
                              <td className="book-title-cell">
                              ...
                              <td>
                                {diffDays != null
                                  ? <span className="due-badge" style={{ background:'#fee2e2', color:'#e11d48' }}>{diffDays}d Overdue</span>
                                  : '—'}
                              </td>
                            </tr>
                          );
                    ...
                    {currentRows.map(loan => {
                      ...
                      return (
                        <div key={loan.id} className="loan-card" style={{ borderLeft:'3px solid #e11d48' }}>
                          <div className="loan-card-header">
                            <div className="loan-card-icon" style={{ background:'linear-gradient(135deg,#e11d48,#be123c)' }}>
                            ...
                            {diffDays != null && (
                              <span className="due-badge" style={{ background:'#fee2e2', color:'#e11d48', flexShrink:0 }}>{diffDays}d Overdue</span>
                            )}
                          </div>
                          <div className="loan-meta-grid">
                            <div className="loan-meta-item">
